...
def literal_eval(node_or_string):
    ...
    def _convert(node):
        ...
        if isinstance(node, Name):
            ...
            if isinstance(node.op, (Add, Sub)) and isinstance(node.right, Num) and isinstance(node.right.n, complex) and isinstance(node.left, Num) and isinstance(node.left.n, (int, long, float)):
                left = node.left.n
                right = node.right.n
