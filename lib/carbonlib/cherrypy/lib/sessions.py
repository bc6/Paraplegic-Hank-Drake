...
class FileSession(Session):
    ...
    def setup(cls, **kwargs):
        ...
        lockfiles = [ fname for fname in os.listdir(cls.storage_path) if fname.startswith(cls.SESSION_PREFIX) if fname.endswith(cls.LOCK_SUFFIX) ]
        if lockfiles:
    ...
    def __len__(self):
        return len([ fname for fname in os.listdir(self.storage_path) if fname.startswith(self.SESSION_PREFIX) if not fname.endswith(self.LOCK_SUFFIX) ])
...
def save():
    ...
    elif isinstance(response.body, types.GeneratorType):
