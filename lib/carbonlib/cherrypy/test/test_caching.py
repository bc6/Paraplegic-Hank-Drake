...
gif_bytes = 'GIF89a\x01\x00\x01\x00\x82\x00\x01\x99"\x1e\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00,\x00\x00\x00\x00\x01\x00\x01\x00\x02\x03\x02\x08\t\x00;'
...
class CacheTest(helper.CPWebCase):
    ...
    def test_antistampede(self):
        ...
        self.getPage('/clear_cache?path=' + urllib.quote('/long_process?seconds=%d' % SECONDS, safe=''))
        self.assertStatus(200)
        ...
        self.assertEqualDates(start, datetime.datetime.now(), seconds=SECONDS + 1)
