...
class Diff(object):
    ...
    def report(self, header = True, dates = False):
        s = []
        if header:
            s.append('Difference in %s from %s to %s:' % (self.before.base_path, self.before.calculated, self.after.calculated))
        for (name, files, show_size,) in [('created', self.created, True), ('deleted', self.deleted, True), ('updated', self.updated, True)]:
            if files:
                s.append('-- %s: -------------------' % name)
                files = files.items()
                files.sort()
...
class Snapshot(IterableUserDict):
    ...
    def __repr__(self):
        return '<%s in %r from %r>' % (self.__class__.__name__, self.base_path, self.calculated or '(no calculation done)')
