...
class ResolveGPCS:
    ...
    def CallUp(self, packet):
        if getattr(packet.destination, 'service', 0):
            if packet.payload[1] == 'MachoBindObject':
                ...
                nodeID = ClockThis('machoNet::GPCS::gpcs.Resolve::CallUp::MachoResolve', sm.GetService(packet.destination.service).MachoResolve, session.GetActualSession())
    ...
    def NotifyUp(self, packet):
        if getattr(packet.destination, 'service', 0):
            nodeID = sm.GetService(packet.destination.service).MachoResolve(session.GetActualSession())
            if type(nodeID) == types.StringType:
