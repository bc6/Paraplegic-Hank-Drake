#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/common/script/localization/pseudoLocalization.py
import localization
...
def Pseudolocalize(englishString):
    ...
    if boot.role == 'client':
        if not hasattr(__builtin__, 'settings'):
            ...
            characterReplacementMethod = prefs.GetValue('characterReplacementMethod', localization.DIACRITIC_REPLACEMENT)
            textExpansionAmount = prefs.GetValue('enableTextExpansion', 0) * prefs.GetValue('textExpansionAmount', 0.0)
    else:
        return englishString
