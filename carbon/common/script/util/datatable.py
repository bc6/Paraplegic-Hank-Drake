...
class DataTableQuery(object):
    ...
    def __init__(self):
        self._DataTableQuery__query_list = {}
    ...
    def filter(self, tq):
        ...
        for x in range(0, len(pos_list)):
            if x < len(pos_list) - 1:
                ...
                self._DataTableQuery__query_list[t1[1]] = tq[t1[0]:t2[0]].replace(t1[1], '').strip()
            else:
                t1 = pos_list[x]
                self._DataTableQuery__query_list[t1[1]] = tq[int(t1[0]):].replace(t1[1], '').strip()
    ...
    def query(self, table_def, table_rows):
        if len(self._DataTableQuery__query_list) == 0:
            return (table_def, table_rows)
        if self._DataTableQuery__query_list.has_key('SELECT'):
            sel = self._DataTableQuery__query_list['SELECT'].replace(' ', '').split(',')
            skip_idx = []
            columns = {}
            ...
            if self._DataTableQuery__query_list.has_key('ORDER BY'):
                idx = columns[self._DataTableQuery__query_list['ORDER BY']]
            ...
            if len(skip_idx) > 0:
                ...
                table_rows = t_rows
        limit = int(self._DataTableQuery__query_list['LIMIT']) if self._DataTableQuery__query_list.has_key('LIMIT') else len(table_rows)
        offset = 0 if not self._DataTableQuery__query_list.has_key('OFFSET') else int(self._DataTableQuery__query_list['OFFSET'])
...
class DataTable(object):
    ...
    def __init__(self, table_def = None, table_rows = [], version = '0.6', tqx = None, tq = None):
        self._DataTable__allowedColumn = ['boolean',
         'number',
         'string',
         'date',
         'datetime',
        ...
        if tq is not None and table_def is not None:
            ...
            (self._DataTable__table_def, self._DataTable__rows,) = tableQuery.filter(tq).query(table_def, table_rows)
        else:
            self._DataTable__table_def = table_def
            self._DataTable__rows = table_rows
        self._DataTable__version = version
        self._DataTable__tqxdict = {}
        if tqx is not None:
            for x in tqx.split(';'):
                args = x.split(':')
                self._DataTable__tqxdict[args[0]] = args[1]

        self._DataTable__errors = []
        self._DataTable__output = []
    ...
    def toRss(self, title, description, link):
        ...
        for r in self._DataTable__rows:
            if len(r) is not 5:
    ...
    def toDataTable(self):
        reply = []
        reply.append(self._DataTable__Column(self._DataTable__table_def))
        reply.append(',')
        reply.append(self._DataTable__Rows(self._DataTable__rows))
        return self._DataTable__Wrapper(reply)



    def toCsv(self):
        reply = []
        reply.append(', '.join(map(lambda x: x[0], self._DataTable__table_def)))
        reply.append('\n'.join([ ', '.join([ unicode(y) for y in x ]) for x in self._DataTable__rows ]))
        return '\n'.join(reply)
    ...
    def toHtmlTable(self):
        reply = []
        reply.append('<table>')
        reply.append('<tr><td>' + '</td><td>'.join(map(lambda x: x[0], self._DataTable__table_def)) + '</td></tr>\n')
        reply.append('\n'.join([ '<tr><td>%s</td></tr>' % '</td><td>'.join(x) for x in self._DataTable__rows ]))
        reply.append('</table>')
        return '\n'.join(reply)
    ...
    def toXml(self):
        ...
        for r in self._DataTable__rows:
            reply.append('<row>')
            row_idx = 0
            for x in self._DataTable__table_def:
    ...
    def __Column(self, columns):
        ...
        for h in self._DataTable__table_def:
            if len(h) != 4 or h[2] not in self._DataTable__allowedColumn:
    ...
    def __Row(self, row, rownum):
        temp = []
        if not row or not isinstance(row, (list, tuple)) or len(row) != len(self._DataTable__table_def):
            raise DataTableException('Table row has problems! %s' % row)
        cidx = 0
        td = self._DataTable__table_def
        for r in row:
            typeName = td[cidx][2]
            if typeName == 'boolean':
    ...
    def __Rows(self, rows):
        ...
        ridx = 0
        for row in rows:
            temp.append(self._DataTable__Row(row, ridx))
            ridx += 1
    ...
    def __Wrapper(self, data):
        version = "version:'%s'" % self._DataTable__version
        reqId = "reqId:'%s'" % self._DataTable__tqxdict['reqId'] if self._DataTable__tqxdict.has_key('reqId') else ''
        status = "status:'ok'" if len(self._DataTable__errors) == 0 else "status:'error'"
        errors = ''.join(['errors:[{', ','.join(self._DataTable__errors), '}]']) if len(self._DataTable__errors) > 0 else ''
        table = 'table:{%s}' % ''.join(data) if len(self._DataTable__errors) == 0 else ''
    ...
    def __Error(self, error_number, error_message):
        self._DataTable__errors.append("{reason:'internal_error',message:'Error in line:%(line)s. Error: %(message)s'}" % {'line': error_number,
