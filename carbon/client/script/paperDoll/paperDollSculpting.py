...
class PaperDollSculpting:
    ...
    def GetMaskAtPixel(self, x, y):
        ...
        if pickedPixelUV[0] > 0.5:
            ...
            buffer = self.bodyZoneMap.LockRect(None, 0, True)
            color = struct.unpack_from('=BBBB', buffer[0], pixel[0] * 4 + pixel[1] * buffer[3])
            mask = color[0]
    ...
    def SetupPickScene(self, doUpdate = True):
        ...
        RT = trinity.device.CreateTexture(32, 32, 1, trinity.TRIUSAGE_RENDERTARGET, trinity.TRIFMT_A8R8G8B8, trinity.TRIPOOL_DEFAULT)
        depth = trinity.TriSurfaceManaged(trinity.device.CreateDepthStencilSurface, 32, 32, trinity.TRIFMT_D24S8, trinity.TRIMULTISAMPLE_NONE, 0, False)
