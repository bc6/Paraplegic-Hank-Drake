...
class SitesSvc(service.Service):
    ...
    def Run(self, *etc):
        ...
        browserSettingsPath = blue.os.settingspath + 'Browser/'
        if not os.path.exists(browserSettingsPath):
            try:
                os.makedirs(blue.os.settingspath + 'Browser/')
            except OSError as e:
        ...
        self.flaggedSitesFile = blue.os.cachepath + 'Browser/flaggedSites.dat'
    ...
    def SaveTrusted(self):
        trustedList = self.GetTrustedSiteList()
        trusted = [ key for (key, value,) in trustedList.iteritems() if value.auto == 0 if value.temporary == 0 if value.trusted == 1 ]
        ignored = [ key for (key, value,) in trustedList.iteritems() if value.auto == 0 if value.temporary == 0 if value.trusted == 0 ]
        f = blue.ResFile()
    ...
    def GetBrowserBlacklist(self):
        if self.systemLists is None:
            self._SitesSvc__PopulateSystemLists()
        if self.systemLists is None:
            return []
