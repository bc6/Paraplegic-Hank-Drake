...
class RegistryService(service.Service):
    ...
    def GetRegisteredWindowState(self, windowID, statename):
    ...
    def GetValidWindows(self, getModals = 0, floatingOnly = False, getHidden = 0):
        ...
        for wnd in self.GetWindows():
            ...
            if getattr(wnd, 'parent', None) is None:
                ...
                print "Window without parent!; that shouldn't be possible. Window: %s" % repr(wnd),
                print wnd.name
                continue
    ...
    def CheckMoveActiveState(self, wnd = None):
        modal = self.GetModalWindow(wnd)
        if modal:
            self.SetFocus(modal)
        elif wnd is None or wnd == self.GetActive():
            validWnds = self.GetValidWindows()
            if validWnds:
                sortedByIndex = uiutil.SortListOfTuples([ (vwnd.parent.children.index(vwnd), vwnd) for vwnd in validWnds ])
                if sortedByIndex:
    ...
    def SetFocus(self, item):
        ...
        if item:
            if self.IsWindow(item):
                ...
                wndAbove = uiutil.GetWindowAbove(item) or uicore.desktop
            item.SetFocus()
            self._RegistryService__SetActive(wndAbove)
            self._focus = item
    ...
    def __SetActive(self, wnd = None):
        ...
        if not wnd or wnd.destroyed:
            wnd = uicore.desktop
        elif self.IsWindow(wnd) and wnd.sr.stack is not None:
    ...
    def GetActive(self):
        focus = self.GetFocus()
        if focus and not focus.destroyed:
            if self.IsWindow(focus):
                return focus
            return uiutil.GetWindowAbove(focus) or uicore.desktop
    ...
    def CrawlForTabstops(self, fromwhere):
        ...
        tabstopgroups = [ wnd for wnd in fromwhere.Find(blueTypeName) if getattr(wnd, 'isTabOrderGroup', None) if uiutil.IsVisible(wnd) ]
        ...
        if current:
            current = current[0]
        if getattr(fromwhere, 'sr', None) is not None:
            fromwhere.sr.tabstopRefs = sorted
    ...
    def Confirm(self, starter = None):
        ...
        if getattr(active, 'Confirm', None) and active != starter:
            ...
            return True
        if getattr(focus, 'OnClick', None):
            uthread.new(focus.OnClick, focus)
            return True
        if focus:
            searchFrom = uiutil.GetWindowAbove(focus) or uicore.desktop
        else:
