#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/client/script/ui/control/xposeCoreClasses.py
import uicls
import log
    ...
    for className, classInstance in uicls.__dict__.items():
        ...
        for k, v in classInstance.__dict__.iteritems():
            ...
            if k not in v.func_code.co_names and k in dir(classInstance.__bases__[-1]):
                if k in fatal:
                    print 'Bad overwrite in class:', className, 'function name:', k
                elif not fatalOnly:
                    print 'Possible bad overwrite in class:', className, 'function name:', k
