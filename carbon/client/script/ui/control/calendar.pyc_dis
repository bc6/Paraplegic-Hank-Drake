#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/carbon/client/script/ui/control/calendar.py
import blue
import uthread
import uiutil
import util
...
class CalendarDayCore(uicls.Container):
    ...
    def GetMenuFunction(self, *args):
        ...
        if not sm.GetService('calendar').IsInPast(self.year, self.month, self.monthday, allowToday=1):
            m += [(uiutil.MenuLabel('/Carbon/UI/Calendar/CreateNewEvent'), self.OpenNewEventWnd)]
        if not self.disabled:
            m += [(uiutil.MenuLabel('/Carbon/UI/Calendar/ViewDay'), self.OpenSingleDayWnd)]
        return m
