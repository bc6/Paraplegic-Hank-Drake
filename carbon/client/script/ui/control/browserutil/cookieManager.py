...
class CookieManager:
        ...
        for each in httpResponse.info().headers:
            ...
            for morsel in cookie.values():
                for oldmorsel in self.cookies[:]:
                    if morsel.key == oldmorsel.key and (morsel['domain'] or host == oldmorsel['domain'] or oldmorsel._CookieManager__host) and morsel['path'] == oldmorsel['path']:
                ...
                if morsel['domain']:
                    ...
                    if not ip.match(host) and host[:(-len(morsel['domain']))].find('.') != -1:
                        ...
                        continue
                morsel._CookieManager__host = host
                morsel._CookieManager__time = blue.os.GetTime()
    ...
    def GetCookie(self, url):
        ...
        for morsel in self.cookies[:]:
            if morsel['max-age'] and blue.os.TimeDiffInMs(morsel._CookieManager__time) / 1000 > morsel['max-age']:
                self.cookies.remove(morsel)
                continue
            if not self.DomainMatches(host, morsel['domain'] or morsel._CookieManager__host):
                continue
