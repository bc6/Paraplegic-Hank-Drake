#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/shared/neocom/attributes.py
import service
import blue
import xtriui
import util
...
class AttributeRespecWindow(uicls.Window):
    ...
    def ConstructLayout(self):
        ...
        for x in xrange(0, 5):
            ...
            label2 = uicls.EveLabelMedium(text='', parent=self.attributePars[x], maxLines=1, left=leftMargin + 28, top=10)
    ...
    def SaveChanges(self, *args):
        ...
        if not allSame:
            ...
            if respecInfo['nextTimedRespec'] is None or respecInfo['nextTimedRespec'] <= blue.os.GetWallclockTime():
                if eve.Message('ConfirmRespec2', {'months': int(const.respecTimeInterval / MONTH)}, uiconst.YESNO) != uiconst.ID_YES:
                    return
            elif freeRespecs > 0:
...
class AttributeRespecEntry(uicls.SE_BaseClassCore):
    ...
    def Startup(self, *args):
        ...
        self.sr.label = uicls.EveLabelSmall(text=localization.GetByLabel('UI/Neocom/NextDNAModification'), parent=self, left=8, top=4, maxLines=1)
        self.sr.respecTime = uicls.EveLabelMedium(text='', parent=self, left=8, top=18, maxLines=1)
        self.sr.numberOfRemaps = uicls.EveLabelMedium(text='', parent=self, left=8, top=38, maxLines=1, state=uiconst.UI_HIDDEN)
