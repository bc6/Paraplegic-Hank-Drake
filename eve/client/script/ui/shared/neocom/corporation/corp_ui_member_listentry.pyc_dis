#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/shared/neocom/corporation/corp_ui_member_listentry.py
import sys
import util
import xtriui
import uix
            ...
            for tabstop in tabstops:
                ...
                if i == 0:
                    if not oldColumns or oldColumns[i] is None:
                        column = uicls.EveLabelMedium(text=rec.name, parent=self, width=width, left=left, top=top, state=uiconst.UI_DISABLED, maxLines=1)
                    ...
                    if not canRecycle:
                        if canEditBase:
                            ...
                            column = uicls.EveLabelMedium(text=text, parent=self, width=width, left=left, top=top, state=uiconst.UI_DISABLED, maxLines=1)
                            maxHeight = max(maxHeight, column.textheight)
                        columnContents.append(column)
                    else:
                        if canEditBase:
                        ...
                        for subColumn in subColumns:
                            ...
                            if not canRecycle:
                                if canEditRole:
                                    ...
                                    column = uicls.EveLabelMedium(text=text, parent=self, width=controlWidth, left=left, top=top, state=uiconst.UI_DISABLED, maxLines=1)
                                    maxHeight = max(maxHeight, column.textheight)
                                columnContents.append(column)
                                left += controlWidth
                            else:
