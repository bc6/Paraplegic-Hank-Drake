...
class SpherePinStack():
    ...
    def CreateSpherePin(self, textureName, layer, radius, transform, isGauge = False, scale = 1.01, color = util.Color.WHITE, display = True, offsetScale = 1.0, xOffset = 0.0, yOffset = 0.0):
        if isGauge:
        ...
        sp.pinRadius = radius
        sp.pinMaxRadius = self.maxRadius
    ...
    def SetLocation(self, surfacePoint):
        self.surfacePoint.SetXYZ(*surfacePoint.GetAsXYZTuple())
...
class ExtractionHeadPin(SpherePinStack):
    ...
    def __init__(self, surfacePoint, transform, ecuPin, headID, radius):
        ...
        self.model3D = self.Get3DModel()
        self.transform = transform
        self.surfacePin = self.CreateSpherePin(textureName='res:/UI/Texture/Planet/drill_pin.dds', layer=headID * 10 + 2, radius=RADIUS_PIN * 0.4, transform=transform, scale=SCALE_ONGROUND * 1.001, color=planetCommon.PLANET_COLOR_EXTRACTOR)
        self.selectionArea = self.CreateSpherePin(textureName='res:/UI/Texture/Planet/arrows_256.dds', layer=headID * 10 + 1, radius=RADIUS_PIN * 1.2, transform=transform, scale=SCALE_ONGROUND * 1.001, color=util.Color(*planetCommon.PLANET_COLOR_EXTRACTOR).SetAlpha(0.7).SetSaturation(0.9).GetRGBA())
        ...
        self.pickArea = self.CreateSpherePin(textureName='res:/UI/Texture/Planet/pin_base.dds', layer=headID * 10 + 2, radius=RADIUS_PIN * 1.9, transform=transform, scale=SCALE_ONGROUND * 1.001, color=(0.0, 0.0, 0.0, 0.01))
