#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/control/fileDialog.py
import xtriui
import uix
import uiutil
import util
...
class FileDialog(uicls.Window):
    ...
    def GetReturnValue(self):
        ...
        if len(selected) == 0 and self.selectionType != uix.SEL_FILES:
    ...
    def _IsEntryInvalid(self, entry):
        return entry.isFolderUp and self.selectionType == uix.SEL_FILES or entry.isDir and self.selectionType == uix.SEL_FILES or not entry.isDir and self.selectionType == uix.SEL_FOLDERS
