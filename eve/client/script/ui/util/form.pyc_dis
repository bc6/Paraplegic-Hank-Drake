#Embedded file name: c:/depot/games/branches/release/EVE-TRANQUILITY/eve/client/script/ui/util/form.py
import service
import uix
import uiutil
import blue
...
class Form(service.Service):
    ...
    def AddLabeltext(self):
        ...
        if label and label != '_hide':
            label = uicls.EveLabelSmall(text=label, parent=self.new, name='label', left=7, width=self.leftPush - 6, top=5)
    ...
    def AddText(self):
        ...
        text = uicls.Label(text=self.type.get('text', ''), parent=self.new, align=uiconst.TOTOP, name='text', padding=(0, 3, 0, 3), fontsize=fontsize, maxLines=1 if bool(self.type.get('tabstops', [])) else None, state=uiconst.UI_NORMAL, tabs=self.type.get('tabstops', []))
