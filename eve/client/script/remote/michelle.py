...
class Michelle(service.Service):
    ...
    def OnFleetStateChange(self, fleetState):
        if self._Michelle__bp is not None:
            self._Michelle__bp.OnFleetStateChange(fleetState)



    def OnDroneStateChange(self, droneID, ownerID, controllerID, activityState, droneTypeID, controllerOwnerID, targetID):
        if self._Michelle__bp is not None:
            self._Michelle__bp.OnDroneStateChange(droneID, ownerID, controllerID, activityState, droneTypeID, controllerOwnerID, targetID)



    def OnDroneActivityChange(self, droneID, activityID, activity):
        if self._Michelle__bp is not None:
            self._Michelle__bp.OnDroneActivityChange(droneID, activityID, activity)
    ...
    def GetFleetState(self):
        if self._Michelle__bp is None:
            return 
        if session.fleetid is None:
            return 
        if self._Michelle__bp.fleetState is None:
            self._Michelle__bp.fleetState = self.GetRemotePark().GetFleetState()
        return self._Michelle__bp.fleetState



    def Refresh(self):
        if self._Michelle__bp is not None:
            self._Michelle__bp.Refresh()
    ...
    def AddBallpark(self, solarsystemID):
        ...
        self.LogInfo('GetBallpark2 object', self._Michelle__bp, 'now has:', sys.getrefcount(self._Michelle__bp), 'references')
        formations = sm.RemoteSvc('beyonce').GetFormations()
        self._Michelle__bp.LoadFormations(formations)
        self.ballNotInParkErrors = {}
        self.handledBallErrors = set()
        self._Michelle__bp.SetBallNotInParkCallback(self.HandleBallNotInParkError)
        sm.ScatterEvent('OnAddBallpark')
        self._Michelle__bp.Start()
        self.bpReady = True
        self.LogNotice('Done adding ballpark', solarsystemID)
        return self._Michelle__bp
    ...
    def GetRemotePark(self):
        if self._Michelle__bp is None:
            return 
        return self._Michelle__bp.remoteBallpark



    def GetBallparkForScene(self, scene):
        self.LogInfo('GetBallpark object', self._Michelle__bp, 'now has:', sys.getrefcount(self._Michelle__bp), 'references')
        if not self._Michelle__bp:
            return None
        if self._Michelle__bp not in self.scenes:
            self.scenes[self._Michelle__bp] = []
        self.scenes[self._Michelle__bp].append(scene)
        return self._Michelle__bp
    ...
    def GetBall(self, id):
        if self._Michelle__bp is not None:
            return self._Michelle__bp.GetBall(id)
        else:
            return 
    ...
    def GetItem(self, id):
        if self._Michelle__bp is not None:
            return self._Michelle__bp.GetInvItem(id)



    def GetDroneState(self, droneID):
        if self._Michelle__bp is not None:
            return self._Michelle__bp.stateByDroneID.get(droneID, None)



    def GetDroneActivity(self, droneID):
        if self._Michelle__bp is not None:
            return self._Michelle__bp.activityByDrone.get(droneID, None)



    def GetDrones(self):
        if self._Michelle__bp is not None:
            return self._Michelle__bp.stateByDroneID
    ...
    def DoDestinyUpdate(self, state, waitForBubble, dogmaMessages = [], doDump = True):
        ...
        if len(timestamps) > 1:
            ...
            if not self._Michelle__bp.hideDesyncSymptoms:
                uthread.new(eve.Message, 'CustomInfo', {'info': 'Desync mismatched updates problem occurred'})
        self._Michelle__bp.FlushState(state, waitForBubble, doDump)
    ...
    def DoDestinyUpdates(self, updates):
        ...
        if self.logInfo and localDump:
            self._Michelle__bp.DumpHistory()
    ...
    def GetCharIDFromShipID(self, shipID):
        if self._Michelle__bp is None:
            return 
        if shipID not in self._Michelle__bp.slimItems:
            return 
        slimItem = self._Michelle__bp.slimItems[shipID]
    ...
    def ProcessDispatchOrders(self, orders):
        (ownersToPrime, tickersToPrime, allyTickersToPrime, stuffToAdd, newState,) = orders
        if ownersToPrime:
            try:
        ...
        for (ballID, slimItem,) in stuffToAdd:
            try:
                if self._Michelle__bp and ballID in self._Michelle__bp.balls:
                    ball = self._Michelle__bp.balls[ballID]
                    if not self._Michelle__bp.slimItems.has_key(ballID):
                        realBalls[ballID] = (ball, slimItem)
                    self._Michelle__bp.slimItems[ballID] = slimItem
            except StandardError:
                self.LogError('In michelle.Dispatcher')
                log.LogException()
                sys.exc_clear()
        ...
        if len(realBalls):
            ...
            ballsSorted = {}
            if self.GetBall(session.shipid) is not None and self._Michelle__bp is not None:
                for (id, v,) in realBalls.iteritems():
                    d = self._Michelle__bp.DistanceBetween(session.shipid, id)
                    while d in ballsSorted:
                        d += 1.0

                    ballsSorted[d] = v

                if len(ballsSorted) != len(realBalls):
                    self.LogError('Michelle::Dispatcher - Distance ball list not equal to original')
                    ballsSorted = realBalls
            else:
                ballsSorted = realBalls
            sm.SendEvent('DoBallsAdded', ballsSorted.values())
            t.PopTimer(timer)
    ...
    def OnAudioActivated(self):
        if self._Michelle__bp is not None:
            self._Michelle__bp.OnAudioActivated()
...
class Park(bluepy.WrapBlueClass('destiny.Ballpark')):
    ...
    def __init__(self):
        ...
        if sm.GetService('machoNet').GetClientConfigVals().get('requestResetOnBindToBallpark'):
            self.RequestReset()
    ...
    def FlushState(self, state, waitForBubble, doDump = True):
        ...
        if len(state) == 0:
            ...
            return 
        if not sm.GetService('machoNet').GetClientConfigVals().get('michelleEnforcesSetStateHistory'):
            if state[0][1][0] == 'SetState':
                self.history = []
        elif state[0][1][0] == 'SetState':
    ...
    def HaveLootRight(self, objectID):
        if self.slimItems.has_key(objectID):
            ...
            if lootRights is not None:
                if None in lootRights:
                    return True
                if session.charid in lootRights:
                    return True
                if session.corpid in lootRights:
                    return True
        return False
